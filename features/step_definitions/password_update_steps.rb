...
When('I fill invalid old password and press Continue') do
...
When('I fill in a password not complex enough and press Continue') do
  allow(AccountsApi).to receive(:update_password).and_raise(
...
When('I fill in a password that was used before and press Continue') do
  allow(AccountsApi).to receive(:update_password).and_raise(
...
When('I fill passwords that do not match and press Continue') do
  fill_in_new_password(password: 'aaaa', password_confirmation: 'bbbb')
  click_button 'Continue'
end

When('I fill in correct old and new password and press Continue') do
