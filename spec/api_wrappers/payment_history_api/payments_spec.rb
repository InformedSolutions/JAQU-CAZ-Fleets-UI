...
describe 'PaymentHistoryApi.payments' do
  subject { PaymentHistoryApi.payments(account_id: account_id, page: page) }

  let(:account_id) { @uuid }
  let(:page) { 5 }
  let(:url) { %r{accounts/#{account_id}/payments\?pageNumber=#{page - 1}} }

  context 'when the response status is 200' do
    before do
      stub_request(:get, %r{accounts/#{account_id}/payments}).to_return(
        status: 200,
