...
describe PaymentHistory::PaginatedPayment, type: :model do
  subject { described_class.new(data) }

  let(:size) { 52 }
  let(:page) { 3 }
  let(:per_page) { 10 }
  let(:payments) { [read_response('payment_history/payments.json')['1']] }
  ...
  describe '.total_payments_count' do
    it 'returns size value' do
      expect(subject.total_payments_count).to eq(size)
    end
  end
