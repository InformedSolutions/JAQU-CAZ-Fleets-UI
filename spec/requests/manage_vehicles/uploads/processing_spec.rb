...
describe 'UploadsController - GET #processing' do
  ...
  context 'correct permissions' do
    ...
    context 'with filename in the session' do
      ...
      before do
        add_to_session(job: { filename: filename, job_name: job_name, correlation_id: correlation_id })
      ...
      it 'calls FleetsApi.job_status with proper params' do
        expect(FleetsApi).to receive(:job_status).with(job_name: job_name, correlation_id: correlation_id)
        subject
      end
      ...
      describe 'job status' do
        ...
        describe 'success' do
          ...
          it 'sets :success flash message' do
            expect(flash[:success]).to eq('You have successfully uploaded 45 to your vehicle list.')
          end
        end
